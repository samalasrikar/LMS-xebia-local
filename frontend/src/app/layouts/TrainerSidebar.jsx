...
export default function TrainerSidebar({ collapsed, setCollapsed }) {
  ...
  useEffect(() => {
    ...
    const handler = (e) => {
      if (e.matches && setCollapsed) setCollapsed(true);
    };
    handler(mql);
  ...
  const sidebarContent = (
    <div className="flex flex-col h-full bg-white">
      ...
      <div className={`flex items-center gap-3 border-b border-slate-200/80 shrink-0 transition-all duration-300 ${collapsed ? "px-0 justify-center py-4" : "px-5 py-[18px]"}`}>
        ...
        <div className={`transition-all duration-300 overflow-hidden ${collapsed ? "w-0 min-w-0 opacity-0 overflow-hidden" : "w-auto opacity-100"}`}>
          <div className="text-[14px] font-bold text-slate-900 tracking-tight leading-none whitespace-nowrap">Xebia LMS</div>
          <div className="text-[9px] text-[#6C1D5F] font-bold tracking-widest uppercase mt-1 whitespace-nowrap">Trainer Portal</div>
        </div>
      </div>
      ...
      <div className="shrink-0 border-t border-slate-200/80 bg-slate-50/50">
        ...
        {!collapsed && (
          <div className="border-t border-slate-200/80 p-3 bg-white">
            <Link to="/trainer/profile" className="flex items-center gap-2.5 cursor-pointer rounded-md p-1 hover:bg-slate-50 transition-colors text-slate-800 hover:text-slate-900 no-underline">
              <img
  ...
  return (
    <>
      ...
      {mobileOpen && (
        <>
          <div
            className="md:hidden fixed inset-0 bg-black/30 backdrop-blur-sm z-45 transition-opacity"
