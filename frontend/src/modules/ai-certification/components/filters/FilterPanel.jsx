...
import { YEARS, QUARTERS, REGIONS, DEPARTMENTS, BUSINESS_UNITS, PRACTICES } from "../../utils/mockData";

export default function FilterPanel({
  filters = {},
  setFilters,
  onReset,
  // State simulation toggles
  isLoading,
  setIsLoading,
  isError,
  setIsError,
  ...
  return (
    <div className="bg-white p-5 rounded-3xl border border-[#d5c1cc]/80 shadow-[0_4px_12px_-4px_rgba(108,29,95,0.05)] space-y-4">
      {/* Title & Reset Row */}
      <div className="flex flex-wrap justify-between items-center gap-3 pb-3 border-b border-[#d5c1cc]/30">
        ...
        {/* State Simulator Badge & Toggles (for developers/reviewers) */}
        <div className="flex flex-wrap items-center gap-2">
          <span className="text-[10px] uppercase font-bold text-[#83727c] mr-2">Simulate UI States:</span>
          
          <button
            onClick={() => setIsLoading(!isLoading)}
            className={`
              inline-flex items-center gap-1 text-[11px] font-bold px-2.5 py-1 rounded-lg border transition-all cursor-pointer
              ${isLoading 
                ? "bg-[#6C1D5F] border-[#6C1D5F] text-white shadow-sm" 
                : "bg-slate-50 border-slate-200 text-slate-600 hover:bg-slate-100"}
            `}
            title="Toggle Loading Skeleton"
          >
          ...
          <button
            ...
            className={`
              ...
              ${isEmpty 
                ...
                : "bg-slate-50 border-slate-200 text-slate-600 hover:bg-slate-100"}
            `}
            title="Toggle Empty Results State"
          >
            <Sparkles size={11} />
            <span>Empty</span>
          </button>
          ...
          <button
            onClick={onReset}
            className="
              inline-flex items-center gap-1.5 text-xs font-bold text-[#83727c] hover:text-[#6C1D5F]
              bg-slate-50 hover:bg-[#6C1D5F]/5 border border-slate-200 px-3 py-1.5 rounded-xl
