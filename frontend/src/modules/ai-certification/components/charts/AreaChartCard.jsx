...
export default function AreaChartCard({
  ...
  data = [],
  xKey = "month",
  areas = [], // array of { key: string, color: string, name: string }
  height = 300
}) {
  return (
              ...
              <defs>
                {areas.map((area, idx) => (
                  <linearGradient key={`grad-${area.key}`} id={`grad-${area.key}`} x1="0" y1="0" x2="0" y2="1">
                    <stop offset="5%" stopColor={area.color || "#6C1D5F"} stopOpacity={0.4} />
                    <stop offset="95%" stopColor={area.color || "#6C1D5F"} stopOpacity={0.0} />
                  </linearGradient>
                ))}
              </defs>
