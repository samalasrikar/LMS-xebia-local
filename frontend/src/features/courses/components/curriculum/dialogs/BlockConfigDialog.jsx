...
import { Plus, X, FileText, HelpCircle, Calendar as CalendarIcon, Video } from "lucide-react";
...
export default function BlockConfigDialog({
    blockConfigOpen,
    setBlockConfigOpen,
    blockConfigType,
    handleSaveBlock,
    ...
    // Block extra fields & state setters
    headingText,
    setHeadingText,
    quoteText,
    setQuoteText,
    ...
    const handleVideoUpload = async (e) => {
        ...
        if (!file) return;
        try {
            const contentServiceMod = (await import("@/features/content-library/services/contentService")).default;
    ...
    return (
        <Dialog open={blockConfigOpen} onOpenChange={setBlockConfigOpen}>
            <DialogContent className="w-[95vw] sm:max-w-5xl rounded-2xl shadow-2xl bg-white border border-slate-200 p-0 flex flex-col max-h-[90vh] overflow-hidden">
                {/* Header */}
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col text-left">
                            ...
                            <p className="text-[11px] text-slate-400 font-medium leading-none mt-1">
                                ...
                                {blockConfigType === "quiz" && "Build your quiz questions"}
                                {blockConfigType === "assignment" && "Define assignment details and parameters"}
                            </p>
                        </div>
                    </div>
                </header>
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-5">
                    ...
                    {blockConfigType === "video" && (
                        <div className="space-y-4 text-left">
                            ...
                            <div className="space-y-1.5">
                                <Label className="text-[12px] font-bold text-slate-500">Or Upload Video File</Label>
                                <input type="file" ref={videoInputRef} onChange={handleVideoUpload} accept="video/*" className="hidden" />
                                {videoUrl && (videoUrl.startsWith("/uploads/") || (uploadedFileName && !videoUrl.includes("youtube.com") && !videoUrl.includes("vimeo.com"))) ? (
                                    <div className="border border-emerald-200 rounded-xl p-3 bg-emerald-50/30 flex items-center justify-between gap-3">
                                        <div className="flex items-center gap-2">
                                            <div className="p-2 bg-emerald-100 rounded-lg"><Video size={14} className="text-emerald-700" /></div>
                                            <div>
                                                <p className="text-xs font-bold text-slate-800 truncate max-w-[240px]">{uploadedFileName || "video.mp4"}</p>
                                                <p className="text-[10px] text-emerald-700 font-bold">Attached</p>
                                            </div>
                                        </div>
                                        <button type="button" onClick={() => { handleVideoUrlChange(""); setUploadedFileName(""); if (videoInputRef.current) videoInputRef.current.value = ""; }} className="px-2.5 py-1 text-slate-500 hover:text-red-600 hover:bg-red-50 rounded-lg text-[10px] font-bold border border-slate-200 bg-white cursor-pointer">
                                            Remove
                                        </button>
                                    </div>
                                ) : (
                                    <div onClick={() => videoInputRef.current?.click()} className="border-2 border-dashed border-slate-200 hover:border-[#6C1D5F] rounded-xl p-6 text-center hover:bg-slate-50/50 transition-all cursor-pointer space-y-2">
                                        <div className="w-10 h-10 rounded-full bg-[#6C1D5F]/5 flex items-center justify-center text-[#6C1D5F] mx-auto"><Video size={18} /></div>
                                        <p className="text-xs font-bold text-slate-700">Click to upload Video</p>
                                        <p className="text-[10px] text-slate-400">Video files only (Max. 200MB)</p>
                                    </div>
                                )}
                            </div>
                        ...
                        <div className="space-y-4 text-left">
                            ...
                            {quoteText && (
                                <div className="space-y-1.5">
                                    ...
                                    <div className="p-4 border-l-4 border-[#6C1D5F] bg-slate-50/80 rounded-r-xl italic text-slate-600 text-xs text-left">
                                        "{quoteText}"
                                    </div>
                                </div>
                            )}
                    ...
                    {blockConfigType === "divider" && (
                        <div className="space-y-4 text-left">
                                ...
                                <div className="flex gap-2">
                                    {["solid", "dashed", "dotted"].map((style) => (
                                        <button
                                            ...
                                            className={`flex-1 py-2 text-[12px] font-bold rounded-xl border capitalize cursor-pointer ${dividerStyle === style
                            ...
                            <div className="p-6 bg-slate-50 rounded-xl flex items-center justify-center">
                                <div className={`w-full h-0.5 bg-slate-300 ${dividerStyle === "dashed" ? "border-t-2 border-dashed border-slate-300 bg-transparent h-0" :
                        ...
                        <div className="space-y-6 text-left">
                            ...
                            {quizQuestions.length === 0 ? (
                                <div className="py-8 text-center border border-dashed border-slate-200 rounded-xl bg-slate-50/50">
                                    <HelpCircle size={24} className="mx-auto mb-2 text-slate-350 pointer-events-none" />
                                        ...
                                        <div key={qIdx} className="p-4 border border-slate-200 rounded-xl bg-slate-50/30 relative space-y-3">
                                            <div className="flex items-center justify-between">
                                                ...
                                                <button
                                                    ...
                                                    className="text-red-500 hover:text-red-700 text-xs font-semibold bg-transparent border-none cursor-pointer"
                                                >
                                                    Remove
                                                </button>
                                            </div>
                                            ...
                                            <div className="space-y-2">
                                                <div className="flex items-center justify-between">
                                                    ...
                                                    <button
                                                        ...
                                                        className="text-[10.5px] font-bold text-[#6C1D5F] hover:underline bg-transparent border-none cursor-pointer"
                                                    >
                                                        + Add Option
                                                    </button>
                                                </div>
